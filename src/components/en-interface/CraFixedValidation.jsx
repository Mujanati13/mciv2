...
const CraFixedValidation = () => {
  ...
  const fetchCraList = async () => {
    ...
    try {
      ...
      if (response.data?.status) {
        ...
        console.log("CRA data up to current day:", filteredData.length, "entries");
        
        // Pre-process the data to ensure all records have valid IDs
        const processedData = craData.map((item, index) => {
          let craId = item.id_imputation || item.id || item.key;
          
          if (!craId) {
            const periode = item.période || "";
            const jour = item.jour || "";
            const consultantId = item.consultant?.id || item.id_consultan || "";
        ...
        setCraList(processedData);
        setFilteredCraList(filteredData.map((item, index) => {
          let craId = item.id_imputation || item.id || item.key;
  ...
  // Extract unique filter options
  const extractFilterOptions = (craData) => {
  ...
  const handleValidation = async (approved, craRecord = null) => {
    setLoading(true);
    
    try {
      const esnId = localStorage.getItem("id");
      
      // Use the passed record if provided, otherwise use the selectedCra from state
      // This is the key change - using a direct reference instead of relying on state
      const recordToValidate = craRecord || selectedCra;
      
      if (!esnId || !recordToValidate) {
        message.error("Information de connexion ou de CRA non trouvée");
        setLoading(false);
        return;
      }
      
      console.log("Record to validate:", recordToValidate);
      
      // Get the ID from the record
      let craId = recordToValidate.id_imputation || recordToValidate.id || recordToValidate.key;
      
      if (!craId) {
        message.error("ID d'imputation CRA non trouvé");
        console.error("Missing CRA ID - Data available:", recordToValidate);
        setLoading(false);
        return;
      }
      ...
      const updatedCraData = {
        période: recordToValidate.période,
        jour: recordToValidate.jour || 1,
        Durée: recordToValidate.Durée || "0",
        type: recordToValidate.type || "travail",
        id_consultan: recordToValidate.id_consultan || recordToValidate.consultant?.id,
        id_esn: parseInt(esnId),
        id_client: recordToValidate.id_client || recordToValidate.client?.id,
        id_bdc: recordToValidate.id_bdc || recordToValidate.project?.id,
      ...
      console.log(`Sending API request to endpoint: ${Endponit()}/api/cra_imputation/${craId}/`);
      ...
      // Try POST as a fallback if we get a 404 or ID error
      if (error.message && error.message.includes("ID") || 
          (error.response && error.response.status === 404)) {
        
        const recordToValidate = craRecord || selectedCra;
        
        if (recordToValidate && recordToValidate.période) {
          try {
            const esnId = localStorage.getItem("id");
            const token = localStorage.getItem("token");
            ...
            console.log("Trying POST as fallback with data:", directValidationData);
        ...
        message.error("Erreur d'identification du CRA. Veuillez rafraîchir la page et réessayer.");
      } else if (error.response && error.response.status === 404) {
        message.error("CRA non trouvé sur le serveur. Veuillez rafraîchir la page et réessayer.");
      } else {
        message.error("Impossible de valider l'imputation CRA. Erreur: " + (error.message || "Inconnue"));
      }
    } finally {
      setLoading(false);
    }
          ...
          {record.statut !== CRA_STATUS.VALIDE && (
            <>
              <Button
                ...
                icon={<CheckOutlined />}
                onClick={() => {
                  // Capture the record in a closure
                  const recordToValidate = {
                    ...record,
                    id: record.id || record.id_imputation || record.key,
                    id_imputation: record.id_imputation || record.id || record.key,
                    key: record.key || record.id_imputation || record.id
                  };
                  
                  console.log("Validating record:", recordToValidate);
                  ...
                  Modal.confirm({
                    ...
                    cancelText: "Annuler",
                    onOk: () => {
                      // Use the captured record directly, not the state
                      return handleValidation(true, recordToValidate);
                    },
                  });
                }}
              >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  console.log("Rejecting record:", recordToReject);
                  ...
                  Modal.confirm({
                    ...
                    onOk: () => {
                      ...
                      // Use the captured record directly
                      return handleValidation(false, recordToReject);
                    },
                  });
                }}
              >
          ...
          selectedCra.statut !== CRA_STATUS.VALIDE && (
            <Space>
              <Button
                ...
                icon={<CheckOutlined />}
                onClick={() => {
                  // Create an enhanced copy of selectedCra to ensure all IDs are set
                  const recordToValidate = {
                    ...selectedCra,
                    id: selectedCra.id || selectedCra.id_imputation || selectedCra.key,
                    id_imputation: selectedCra.id_imputation || selectedCra.id || selectedCra.key,
                    key: selectedCra.key || selectedCra.id_imputation || selectedCra.id
                  };
                  ...
                  Modal.confirm({
                    ...
                    okText: "Valider",
                    cancelText: "Annuler",
                    onOk: () => handleValidation(true, recordToValidate),
                  });
                }}
              >
                Valider
...
export default CraFixedValidation;
