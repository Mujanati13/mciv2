...
const PartenariatInterface = () => {
  ...
  const fetchPartenariats = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/PartenariatClients/?clientId=${clientId}`,
        axiosConfig
      );
  ...
  const handleOk = async () => {
    try {
      ...
      if (editingId) {
        await axios.put(
          `${API_BASE_URL}/partenariats/${editingId}`,
          formattedValues,
          axiosConfig
        );
        ...
        <Card.Meta
          ...
          description={
            <Space direction="vertical" size="small" style={{ width: "100%" }}>
              ...
              <Tag color={item.statut === "Actif" ? "green" : "red"}>
                {item.statut}
              </Tag>
    ...
    <div style={{ padding: "0px" }}>
      <Card>
        ...
        <Modal
          ...
          width={800}
        >
          <Form form={form} layout="vertical" name="partenariatForm">
            <Row gutter={16}>
              {/* <Col span={12}>
                <Form.Item
                  name="id_client"
              ...
              <Col span={12}>
                <Form.Item
                  ...
                  label="ESN"
                  rules={[
                    { required: true, message: "Veuillez sélectionner l'ESN" },
                  ]}
                >
        ...
        <Modal
          title={
            <Space>
              <EyeOutlined /> Détails du partenariat
            </Space>
          }
                  ...
                  <Col span={24}>
                    <Card>
                      <Space direction="vertical" size="small">
                        <Title level={4}>
                          {selectedRecord.client.raison_sociale}
                        </Title>
                        <Space>
                  ...
                  <Col span={12}>
                    <Card title="Informations Client" size="small">
                      <Space direction="vertical">
                        <Text>
                          <strong>SIRET:</strong> {selectedRecord.client.siret}
                        </Text>
                        <Text>
                          <strong>Email:</strong>{" "}
                          {selectedRecord.client.mail_contact}
                        </Text>
                        <Text>
                          <strong>Téléphone:</strong>{" "}
                          {selectedRecord.client.tel_contact}
                        </Text>
                        <Text>
                          <strong>Adresse:</strong>{" "}
                          {selectedRecord.client.adresse},{" "}
                          {selectedRecord.client.cp}{" "}
                          {selectedRecord.client.ville}
                        </Text>
                      </Space>
                    </Card>
                  </Col>
                    ...
                    <Card title="Informations ESN" size="small">
                      <Space direction="vertical">
                        <Text>
                          ...
                          {selectedRecord.esn.Raison_sociale}
                        </Text>
                        <Text>
                          <strong>SIRET:</strong> {selectedRecord.esn.SIRET}
                        </Text>
                        <Text>
                          <strong>Email:</strong>{" "}
                          {selectedRecord.esn.mail_Contact}
                        </Text>
                        <Text>
                          <strong>Téléphone:</strong>{" "}
                          {selectedRecord.esn.Tel_Contact}
                        </Text>
                      </Space>
                    </Card>
                  </Col>
                      ...
                      <Space direction="vertical">
                        <Space>
                          ...
                          <Text>
                            ...
                            {formatDate(selectedRecord.date_debut)} au{" "}
                            {formatDate(selectedRecord.date_fin)}
                          </Text>
                        </Space>
